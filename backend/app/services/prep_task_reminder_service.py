...
def list_incomplete_prep_tasks_due_within(
    db: Session,
    *,
    days: int | None = None,
    ...
    tasks = list(
        db.scalars(
            select(PrepTask)
            .where(PrepTask.due_date >= now)
            .where(PrepTask.due_date <= window_end)
            .options(
                selectinload(PrepTask.checklist_items),
                selectinload(PrepTask.group),
                selectinload(PrepTask.event),
            )
...
def scan_and_notify_prep_tasks_due_soon(
    ...
    *,
    ...
    due_soon_tasks = list_incomplete_prep_tasks_due_within(db, as_of=as_of)
    emails_queued = 0
    ...
    for task in due_soon_tasks:
        if task.assignee_id is None:
            ...
            logger.info(
                "Prep task due soon with no assignee task_id=%s event_id=%s group=%s due=%s",
                task.id,
                task.event_id,
                task.group.group_name if task.group else "unknown",
        ...
        if assignee is None:
            skipped_unassigned += 1
            logger.warning(
                "Prep task assignee missing task_id=%s assignee_id=%s",
                task.id,
                task.assignee_id,
            )
            continue
        ...
        if due_soon_reminder_exists(
            ...
            logger.info(
                "Prep task due-soon reminder already sent task_id=%s assignee_id=%s",
                task.id,
                assignee.id,
            )
            continue
    ...
    logger.info(
        "Prep task due-soon scan complete scanned=%s emails_queued=%s "
